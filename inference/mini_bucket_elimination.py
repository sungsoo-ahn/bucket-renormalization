        ...
        for t in range(len(self.model.variables)):
            ...
            for cand_var in uneliminated_variables:
                ...
                for facs in bucket_for_[cand_var]:
                    mini_bucket = next((mb for mb in candidate_mini_buckets_for_[cand_var] if self.get_bucket_size(mb + [facs], eliminated = eliminated_variables + [cand_var]) < ibound), False)
                    if mini_bucket:
