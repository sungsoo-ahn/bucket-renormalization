...
class BucketRenormalization(MiniBucketElimination):
    ...
    def optimize(self):
        for var in reversed(self.renormalized_elimination_order):
            if var in self.replications.keys():
                ...
                new_projector = Factor(name = default_factor_name(),
                                       variables = [var],
    ...
    def get_logZ(self):
        be = BucketElimination(self.renormalized_model)
        logZ = self.base_logZ
        logZ += be.run(elimination_order_method = 'given',
                       elimination_order = self.renormalized_elimination_order)
    ...
    def _get_svd_projector(self, factor, variable):
        ...
        flattened_factor_log_values = factor.log_values.reshape(factor.get_cardinality_for_(variable), -1)

        if np.isfinite(np.max(flattened_factor_log_values)):
            flattened_factor_values = np.exp(flattened_factor_log_values
                                             - np.max(flattened_factor_log_values))
        else:
            raise ValueError()
