...
class BeliefPropagation():
    def __init__(self, model):
        ...
        self.factors_adj_to_ = {var: self.model.get_adj_factors(var) for var in self.model.variables}

        self.messages = dict()
        for fac in model.factors:
            for var in fac.variables:
                self.messages[(fac, var)] = Factor.initialize_with_(default_message_name(),
                                                                   [var],init_np_func,
        ...
        for fac in model.factors:
            for var in fac.variables:
                self.messages[(var, fac)] = Factor.initialize_with_(default_message_name(),
                                                                   [var], init_np_func,
                                                                   model.get_cardinality_for_(var))
    ...
    def run(self, max_iter = 1000, converge_thr = 1e-5, damp_ratio = 0.1):
        ...
        for var in self.model.variables:
            self.belief[var] = (product_over_(*self._message_to_(var))
                                .normalize(inplace = False))

        for fac in self.model.factors:
            self.belief[fac] = (product_over_(fac, *self._message_to_(fac))
                                .normalize(inplace = False))
    ...
    def _update_messages(self, damp_ratio):
        ...
        for fac in factor_order:
            for var in fac.variables:
                next_message = (product_over_(fac, *[msg for msg in self._message_to_(fac, except_objs = [var])])
                          .marginalize_except_([var], inplace = False)
                          .normalize(inplace = False))
                self.messages[(fac, var)] = (damp_ratio * self.messages[(fac, var)]
        ...
        for var in variable_order:
            for fac in self.factors_adj_to_[var]:
                ...
                if messages_to_var_except_fac:
                    next_message = product_over_(*messages_to_var_except_fac).normalize(inplace = False)
                    self.messages[(var, fac)] = (damp_ratio * self.messages[(var, fac)]
                                                 + (1 - damp_ratio) * next_message)


    def _is_converged(self, converge_thr, messages, new_messages):
        for var in self.model.variables:
            blf = (product_over_(*[messages[(fac, var)] for fac in self.factors_adj_to_[var]])
                   .normalize(inplace = False))
            new_blf = (product_over_(*[new_messages[(fac, var)] for fac in self.factors_adj_to_[var]])
                       .normalize(inplace = False))
    ...
    def _message_to_(self, obj, except_objs = []):
        if obj in self.model.factors:
            ...
            return [self.messages[(fac, obj)] for fac in self.factors_adj_to_[obj] if fac not in except_objs]
        else:
    ...
    def __init__(self, model, ibound):
        ...
        while unelminated_variables:
            ...
            if get_key(var) == 1:
                ...
                mini_buckets = []
                for fac in facs:
                    mini_bucket = next((mb for mb in mini_buckets if get_bucket_size(mb + [fac]) < ibound), False)
                    if mini_bucket:
